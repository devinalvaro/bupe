...
export async function createEpubReader(data: Uint8Array): Promise<EpubReader> {
  const dataReader = new zip.Uint8ArrayReader(data);
  const zipReader = new ZipReader(dataReader);
  const zipEntries = await zipReader.getEntries();

  const container = getContainer(zipEntries);

  const rootfile = pipe(
    container,
    andThen((container) => getRootfile(zipEntries, container)),
  );
  ...
  return {
    ...
    toc: pipe(
      ...
      andThen(([container, rootfile]) => getToc(zipEntries, container, rootfile)),
    ),
    getContent: (src: string) =>
      pipe(
        container,
        andThen((container) => getContent(zipEntries, container)(src)),
      ),
